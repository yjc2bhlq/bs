<template>
	<div class="addEdit-block" :style='{"padding":"30px","backgroundAttachment":"fixed","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20220722/73b1ccbab1284723be5e9e82f00858e4.png)","display":"flex","backgroundSize":"cover","backgroundPosition":"center center","backgroundRepeat":"no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		<el-form
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="分类" prop="fenlei">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="分类" prop="fenlei">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="拾得地点" prop="shidedidian">
					<el-input v-model="ruleForm.shidedidian" placeholder="拾得地点" clearable  :readonly="ro.shidedidian"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="拾得地点" prop="shidedidian">
					<el-input v-model="ruleForm.shidedidian" placeholder="拾得地点" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="拾得时间" prop="shideshijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.shideshijian" label="拾得时间" prop="shideshijian">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="物品描述" prop="wupinmiaoshu">
					<editor 
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.wupinmiaoshu" label="物品描述" prop="wupinmiaoshu">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.wupinmiaoshu"></span>
                </el-form-item>
			<el-form-item :style='{"width":"100%","padding":"0","margin":"0"}' class="btn">
				<el-button :style='{"border":"0","cursor":"pointer","padding":"0","margin":"0 20px 0 0","outline":"none","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"linear-gradient(180deg, #A2CDE0 0%, #76A5FF 100%)","width":"128px","lineHeight":"40px","fontSize":"14px","height":"40px"}'  v-if="type!='info'" type="primary" class="btn-success" @click="onSubmit">提交</el-button>
				<el-button :style='{"border":"0px solid rgba(64, 158, 255, 1)","cursor":"pointer","padding":"0","margin":"0","outline":"none","color":"#fff","borderRadius":"4px","background":"linear-gradient(180deg, #A2CDE0 0%, #76A5FF 100%)","width":"128px","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="type!='info'" class="btn-close" @click="back()">取消</el-button>
				<el-button :style='{"border":"0px solid rgba(64, 158, 255, 1)","cursor":"pointer","padding":"0","margin":"0","outline":"none","color":"#fff","borderRadius":"4px","background":"linear-gradient(180deg, #A2CDE0 0%, #76A5FF 100%)","width":"128px","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="type=='info'" class="btn-close" @click="back()">返回</el-button>
			</el-form-item>
		</el-form>
...
export default {
	...
	methods: {
		...
		init(id,type) {
            ...
            this.zhuangtaiOptions = "正在寻找,正在认领,已找到,已认领,交接中".split(',')
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.wupinmiaoshu = this.ruleForm.wupinmiaoshu.replace(reg,'../../../bs/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhaolingqishi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhaolingqishiCrossAddOrUpdateFlag = false;
						 this.parent.search();

	...
	<div class="detail-preview" :style='{"width":"1200px","padding":"0 0 20px","margin":"20px auto 0","position":"relative"}'>
		<div class="attr" :style='{"padding":"20px","borderColor":"#52b646","borderRadius":"12px","flexWrap":"wrap","background":"#fff","borderWidth":"3px","display":"flex","position":"relative","borderStyle":"dotted dashed solid double","justifyContent":"center"}'>
			...
			<div class="info" :style='{"width":"50%","padding":"10px","margin":"0 2% 0 0px","background":"#fff","order":"2"}'>
				...
				<div class="item" :style='{"border":"1px solid #88d67e","padding":"6px 10px","boxShadow":"inset 0px 0px 48px 0px #def5db","margin":"0 0 10px 0","borderRadius":"4px","background":"#fff","display":"flex","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"right","background":"none","width":"120px","fontSize":"14px","lineHeight":"40px","minWidth":"120px"}'>拾得时间</div>
					<div  :style='{"width":"498px","padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666","height":"auto"}'>{{detail.shideshijian}}</div>
				</div>
				<div class="item" :style='{"border":"1px solid #88d67e","padding":"6px 10px","boxShadow":"inset 0px 0px 48px 0px #def5db","margin":"0 0 10px 0","borderRadius":"4px","background":"#fff","display":"flex","justifyContent":"spaceBetween"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(132,218,110,1) 0%, rgba(63,187,33,1) 81%, rgba(82,182,70,1) 100%)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="isAuth('zhaolingqishi','联系我')" @click="onAcross('renlingxinxi','','','')" type="warning">联系我</el-button>
				</div>
			</div>
  ...
  export default {
    ...
    methods: {
      ...
      onAcross(acrossTable,crossOptAudit,statusColumnName,tips,statusColumnValue){
        localStorage.setItem('crossTable',`zhaolingqishi`);
      ...
      storeup(type) {
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'zhaolingqishi', userid: localStorage.getItem('userid')}}).then(res => {
      ...
      getDiscussList(page) {
        this.$http.get('discusszhaolingqishi/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discusszhaolingqishi/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
                this.getDiscussList(1);
                this.$message({
